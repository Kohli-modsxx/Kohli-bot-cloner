...
# ── Global registries (per bot_id) ──
BOT_INSTANCES = {}
BROADCAST_QUEUES = {}

def get_broadcast_queue(bot_id):
    return BROADCAST_QUEUES.setdefault(bot_id, [])

# ─────────────────────────────────────────────────────────────
# MAIN ENTRY — CALLED BY app.py FOR EACH BOT
# ─────────────────────────────────────────────────────────────
async def run_bot_instance(token, owner_id, bot_id):
    # Per-bot config
    DATA_FILE = f"data_{bot_id}.json"
    FIREBASE_URL = "https://glowbet-1b2ce-default-rtdb.firebaseio.com"  # optional
    ...
    BOT_INSTANCES[bot_id] = {"bot": bot, "owner_id": owner_id}
    ...
    def decode_cid(token_str: str) -> str:
        try:
            return f"-100{int(token_str, 16)}"
        except Exception:
            return token_str
    ...
    def build_session_summary(session, reason):
        ...
        return (
            ...
            f"{sep}\n\n"
            f"🖥 𝐒ᴇʀᴠᴇʀ : {server_name}\n"
            f"🕐 𝐒ᴛᴀʀᴛ : <code>{started}</code>\n"
    ...
    async def load_data():
        ...
        if FIREBASE_URL:
            for attempt in range(MAX_RETRIES):
                try:
                    r = await get_http().get(f"{FIREBASE_URL.rstrip('/')}/bot_data_{bot_id}.json")
    ...
    async def save_data(data):
        ...
        if FIREBASE_URL:
            try:
                await get_http().put(
                    f"{FIREBASE_URL.rstrip('/')}/bot_data_{bot_id}.json",
                    json=data
                )
            except Exception:
                pass
    ...
    async def on_my_chat_member_update(update: types.ChatMemberUpdated):
        ...
        if added:
            try:
                ...
                kb.row(types.InlineKeyboardButton("🚀 𝐒ᴛᴀʀᴛ 𝐏ʀᴇᴅɪᴄᴛɪᴏɴ 𝐍ᴏᴡ!", callback_data=f"quickstart_{channel_id}"))
    ...
    HELP_TEXT = (
        ...
        "📊 𝐄ɴɢɪɴᴇs:\n"
        "• ⚔️ 𝐒ᴇʀᴠᴇʀ 1 — Dhruv HTML Engine (Dragon+Chop+Density)\n"
        "• 🤖 𝐒ᴇʀᴠᴇʀ 2 — Adaptive Quant Engine (Ensemble Analysis)\n"
        "• 🔑 𝐒ᴇʀᴠᴇʀ 3 — Seed Hash Decrypter (Mathematical Hashing)\n\n"
        "📞 𝐒ᴜᴘᴘᴏʀᴛ: @xxLEGEND_KOHLI"
    )
    ...
    async def trigger_start(message, user_id):
        ...
        try:
            ...
            if user_id in active_sessions:
                await safe_edit(loading_msg, "⚠️ 𝐒ᴇssɪᴏɴ 𝐀ʟʀᴇᴀᴅʏ 𝐑ᴜɴɴɪɴɢ!\nPehle '🛑 Stop Prediction' karein.")
                return
            ...
            await safe_edit(loading_msg,
                f"🎯 𝐒ᴇʟᴇᴄᴛ 𝐒ᴇʀᴠᴇʀ\n{'━'*20}\n"
                f"⚔️ 𝐒ᴇʀᴠᴇʀ 1 — Dragon Line + Chop + Density\n"
                f"🤖 𝐒ᴇʀᴠᴇʀ 2 — Adaptive Quant Multi-Heuristic Engine\n"
                f"🔑 𝐒ᴇʀᴠᴇʀ 3 — Seed Hash Decrypter (SHA256 Win Predictor)\n"
                f"{'━'*20}\n𝐂ʜᴏᴏsᴇ 𝐒ᴇʀᴠᴇʀ:",
                reply_markup=kb)
        except Exception as e:
            await safe_edit(loading_msg, f"❌ 𝐄ʀʀᴏʀ: <code>{str(e)[:150]}</code>")

    # ── COMMANDS ──
    @bot.message_handler(commands=['start'])
    async def cmd_start(message: types.Message):
        user_id = message.from_user.id
        kb = admin_kb() if user_id == owner_id else user_kb()
        await bot.send_message(message.chat.id,
            f"🎯 𝐖ᴇʟᴄᴏᴍᴇ, {message.from_user.first_name}!\n\n{'━'*22}\n"
            f"🤖 𝐍ᴀɪᴛɪᴋ 𝐕𝐈𝐏 𝐏ʀᴇᴅɪᴄᴛɪᴏɴ 𝐁ᴏᴛ v4.0\n"
            f"⚔️ 𝐒ᴇʀᴠᴇʀ 1: Dragon HTML Engine\n"
            f"🤖 𝐒ᴇʀᴠᴇʀ 2: Adaptive Quant Engine\n"
            f"🔑 𝐒ᴇʀᴠᴇʀ 3: Seed Hash Decrypter\n{'━'*22}\n\n𝐔sᴇ ᴏᴘᴛɪᴏɴs ʙᴇʟᴏᴡ:",
            parse_mode="HTML", reply_markup=kb)
    ...
    async def cmd_add(message: types.Message):
        ...
        status_msg = await bot.send_message(message.chat.id, "🔄 𝐕ᴇʀɪꜰʏɪɴɢ...", parse_mode="HTML")
        try:
    ...
    async def on_text(message: types.Message):
        ...
        if text == "📊 My Channels":
            ...
            if not channels:
                ...
                return
            resp = f"📊 𝐘ᴏᴜʀ 𝐂ʜᴀɴɴᴇʟs ({len(channels)})\n{'━'*22}\n\n"
        ...
        if text == "📢 Broadcast" and user_id == owner_id:
            user_states[owner_id] = "waiting_broadcast"
            await bot.send_message(message.chat.id,
                "📢 Enter the message to broadcast.\n"
                "It will be sent to ALL user DMs and ALL connected channels/groups.")
            return

        if text == "🔧 System Stats" and user_id == owner_id:
        ...
        if state == "waiting_broadcast" and user_id == owner_id:
            ...
            await safe_edit(status_msg,
                ...
                f"👤 𝐔sᴇʀ 𝐃𝐌s\n  ✅ Success: {ok_dm}\n  ❌ Failed: {fail_dm}\n\n"
                f"📢 𝐂ʜᴀɴɴᴇʟs/𝐆ʀᴏᴜᴘs\n  ✅ Success: {ok_ch}\n  ❌ Failed: {fail_ch}\n\n"
                f"📊 𝐉ᴏᴜʀɴᴇʏ 𝐃ᴇʟɪᴠᴇʀᴇᴅ: {ok_dm + ok_ch}")
            return
    ...
    async def on_callback(cq: types.CallbackQuery):
        ...
        try:
            ...
            if data.startswith("quickstart_"):
                ...
                kb.row(types.InlineKeyboardButton("🔑 SERVER 3 — Seed Hash Decrypter", callback_data="server_3"))
                kb.row(types.InlineKeyboardButton("❌ Cancel", callback_data="cancel"))
                await safe_edit(cq.message, "🎯 𝐒ᴇʟᴇᴄᴛ 𝐒ᴇʀᴠᴇʀ\n\nChoose engine:", reply_markup=kb)
                return
            ...
            try:
                await safe_edit(cq.message, f"❌ 𝐄ʀʀᴏʀ: <code>{str(e)[:150]}</code>")
            except Exception:
                pass

    # ── BROADCAST WORKER (from Flask API) ──
    async def broadcast_worker():
        while True:
            try:
                q = get_broadcast_queue(bot_id)
                while q:
                    msg = q.pop(0)
                    try:
                        await run_broadcast(owner_id, msg)
                    except Exception as e:
                        print(f"[{bot_id}] broadcast error: {e}")
                await asyncio.sleep(3)
            except asyncio.CancelledError:
                break
            except Exception:
    ...
    print(f"✅ Bot instance [{bot_id}] ONLINE for owner {owner_id}")
